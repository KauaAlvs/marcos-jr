...
export default function FemininoPage() {
  ...
  // ==========================================
  // BUSCA NO SUPABASE (Filtro por WOMENS)
  // ==========================================
  useEffect(() => {
    async function fetchWomensProducts() {
      try {
        const { data, error } = await supabase
          ...
          `)
          .eq('ativo', true)
          .eq('genero', 'WOMENS'); // FILTRO CRUCIAL AQUI
        ...
        console.error("Erro ao buscar produtos femininos do Supabase:", error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <main className="min-h-screen bg-white text-gray-950 font-sans selection:bg-black selection:text-white">
      ...
      {/* 1. HERO CATEGORIA (FEMININO) */}
      <section className="relative h-[100dvh] lg:h-[100dvh] w-full overflow-hidden bg-gray-900 group">
        <img 
          src="https://ik.imagekit.io/1t4v46udu/BANNER/davinci_editorial_cycling_photograph_of_a_female_cyclist_s.png?updatedAt=1774221857123" 
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-16 lg:p-24 z-10 pointer-events-none max-w-[1600px] mx-auto w-full">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-white text-5xl md:text-7xl lg:text-8xl font-bold uppercase tracking-tighter leading-[0.85]">
              Womens <br/> <span className="font-light italic opacity-90">Collection</span>
            </h1>
            <p className="text-gray-300 mt-6 text-sm md:text-base tracking-widest uppercase max-w-md font-medium">
              Desenvolvido sem atalhos para quebrar seus limites.
            </p>
          </motion.div>
        </div>
      </section>

      {/* 2. GRID DE PRODUTOS FEMININOS */}
      ...
      <footer className="bg-white border-t border-gray-100 font-sans pt-24 pb-12 overflow-hidden">
        <div className="px-4 sm:px-8 lg:px-16 max-w-[1600px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center pt-12 border-t border-gray-100 text-[9px] font-bold tracking-[0.3em] text-gray-400 uppercase">
            <p>© {new Date().getFullYear()} COPÊ SYNDICATE. TODOS OS DIREITOS RESERVADOS.</p>
