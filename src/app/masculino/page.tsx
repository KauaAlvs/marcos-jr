...
export default function MasculinoPage() {
  ...
  // ==========================================
  // BUSCA NO SUPABASE (Filtro por MENS)
  // ==========================================
  useEffect(() => {
    async function fetchMensProducts() {
      try {
        const { data, error } = await supabase
          ...
          `)
          .eq('ativo', true)
          .eq('genero', 'MENS'); // FILTRO CRUCIAL AQUI
        ...
        if (data) {
          const formattedProducts = data.map((p: any) => {
            ...
            return {
              ...
              category: p.categorias?.nome || 'PRODUTO'
            };
          });
        ...
        console.error("Erro ao buscar produtos masculinos do Supabase:", error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <main className="min-h-screen bg-white text-gray-950 font-sans selection:bg-black selection:text-white">
      ...
      {/* 1. HERO CATEGORIA (MASCULINO) */}
      <section className="relative h-[100dvh] lg:h-[100dvh] w-full overflow-hidden bg-gray-900 group">
        <img 
          src="https://ik.imagekit.io/1t4v46udu/BANNER/davinci_editorial_cycling_photograph_of_a_male_cyclist_rid.png?updatedAt=1774221866853" 
          alt="Mens Collection" 
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-16 lg:p-24 z-10 pointer-events-none max-w-[1600px] mx-auto w-full">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-white text-5xl md:text-7xl lg:text-8xl font-bold uppercase tracking-tighter leading-[0.85]">
              Mens <br/> <span className="font-light italic opacity-90">Collection</span>
            </h1>
            <p className="text-gray-300 mt-6 text-sm md:text-base tracking-widest uppercase max-w-md font-medium">
              Engenharia de precisão para atrito e velocidade.
            </p>
          </motion.div>
        </div>
      </section>

      {/* 2. GRID DE PRODUTOS MASCULINOS */}
      <section className="py-24 md:py-32 bg-white">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-16">
          ...
          <div className="flex justify-between items-end mb-16 border-b border-gray-100 pb-6">
            ...
            {/* Espaço para futuros filtros de Categoria / Preço */}
      ...
      {/* 10. FOOTER PREMIUM (Reutilizado da Home) */}
